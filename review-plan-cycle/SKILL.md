...
- **Comment discipline** — the implementer defaults to **no comments**; clearer names and smaller functions are preferred over a comment that explains them. A comment is justified only when it records something the code cannot show — a non-obvious invariant, a why-not-the-obvious-way, a known hazard or workaround with its cause, or a contract a caller must honor — and then it must be **precise**: the fact, not a narration of what the code does, and never commented-out code or references to the plan/task. Before writing any comment the implementer must decide it is **absolutely** necessary by that test; if not, write none. When one is warranted, cap it at ~1–2 lines even in a comment-dense file — matching the file's existing density never licenses a longer comment.
