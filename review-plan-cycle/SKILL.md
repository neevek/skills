...
Run plan-review passes over an implementation **plan** until remaining findings fall below the stop threshold. Planning only — **no code edits, no refactors, no implementation steps executed**; the output is an execution-ready plan with technical and design concerns surfaced early. Sibling `review-fix-cycle` does the same for code diffs.

**REQUIRED SUB-SKILL:** load `review-cycle-core` for the loop mechanics — spawning, the one-shot report contract, triage discipline, the ledger, the stop rule, comment discipline, and ADR / ubiquitous-language discipline. This skill supplies only the plan-specific specializations below.
...
- a **Plan risk model** (see Risk model) naming every relevant risk class; mark an irrelevant class `n/a` with a short reason — never leave it implicit.
...
Mark each finding **accept / reject / defer**, recording why. *Reject* taste disagreements you've considered and settled — a reviewer's architectural preference is not automatically correct. *Defer* anything that's genuinely an implementation-time decision, not a plan blocker.

`needs-discussion` (the reviewer unsure the concern is real) is not a fourth state — resolve it: a feasibility/technical doubt gets at most one more read-only reviewer this pass; a product/scope decision only the user can make (priorities, intent, acceptable trade-offs) means **stop and ask the user** rather than letting reviewers churn on an undecidable point.
...
- When a pass ran **Design It Twice**, record the chosen design and its rationale so the next reviewer can validate the direction.

Before declaring the loop done, run the **Execution-readiness gate**.

## Risk model

Before pass 1, classify the plan against these generic classes; use the classification to choose reviewer scopes and make the first pass adversarial rather than merely broad. For any relevant class, the plan must either address it directly or name it as a risk with an explicit owner/decision — a reviewer treats an unaddressed relevant class as a plan gap.
...
Borrowed from deep-module design so complexity findings are reproducible, not taste; use them in the reviewer prompt, the Design concerns section, and the ledger.
...
- **Interface** — everything a caller must know to use a module correctly: signature *plus* invariants, ordering, error modes, required config, performance characteristics. **Seam** — where that interface lives (its own design decision). **Adapter** — a concrete thing satisfying an interface at a seam.
...
The plan is handed to an implementer who will not infer conventions — state them once, in the plan's **Implementation conventions** field:

- **Comment discipline** — copy the core skill's shared Comment discipline into the plan (implementer defaults to no comments; only a fact the code cannot show; precise, ~1–2 lines, decided on the first write).
- Do not let the plan itself prescribe explanatory comments as a substitute for a clear design — a step only understandable with a comment is a signal to simplify the step.

## Execution-readiness gate (the plan-loop's validation)

A plan can't be compiled; before declaring the loop done, the main session confirms the plan is ready to hand to an implementer:
...
If any is missing, the plan is not execution-ready regardless of finding count; if the gate fails when the Stop rule would otherwise end the loop, do not silently finish — fix the gap in another pass if the cap allows, otherwise stop and report **not converged** with the specific gate gap listed.

## Scopes (for multi-reviewer fan-out)

Split a large plan by concern; fold the relevant ones into a single prompt for a small plan (the prompt must still list the relevant risk classes). For plans with more than one relevant risk class, prefer fan-out by risk class over one generic reviewer.
...
- **UX/interaction** (when the plan includes user-facing flows) — flows, states, error/empty/loading, accessibility implications.
...
Specialize with the plan scope and scope focus; inject the severity rubric and one-shot report contract:
