...
List actionable findings first, ordered by severity per the injected rubric (High/Medium/Low), each with file + line and a one-line justification. Skip style the formatter/linter handles. If unsure a finding is real, mark it `needs-verification` rather than asserting it.
